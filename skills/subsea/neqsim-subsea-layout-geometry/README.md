...
```bash
python -m pip install -e skills/subsea/neqsim-subsea-layout-geometry
# optional: schematic plan-view illustration
python -m pip install -e "skills/subsea/neqsim-subsea-layout-geometry[plot]"
```

## Run Example

```bash
python skills/subsea/neqsim-subsea-layout-geometry/examples/basic_subsea_layout_geometry.py
# schematic map (requires the [plot] extra)
python skills/subsea/neqsim-subsea-layout-geometry/examples/subsea_field_layout_map.py
```

## Run Tests

```bash
python -m pytest skills/subsea/neqsim-subsea-layout-geometry/tests
```
