...
| `trend.series("PT")` returns the wrong location | `names()` repeats a name once per `TRENDDATA` statement and `series(name)` takes the first match | Select on the full triple: scan `trend.variables` for `(name, variable.pipe, variable.section)` and index `trend.values` yourself |
| A severe-slugging comparison is reduced to one peak-to-peak number | A limit cycle is trajectory-sensitive; the instantaneous peak is not reproducible, and a run that damps out can still show a large peak from its start-up transient | Report the cycle **period**, the **P10-P90** band and the number of cycles detected in the window alongside the peak-to-peak. A P10-P90 far below the peak-to-peak means the trace is flat with spikes, not cycling |
| A pipeline-riser case oscillates in OLGA but under-predicts the measured amplitude and period | The gas `SOURCE` sits at the flowline inlet, so the only compressible upstream volume is the flowline itself, while the facility had a gas supply or buffer volume upstream | Model the upstream volume explicitly before comparing against a facility measurement; without it the comparison is still valid **between simulators**, but not against the experiment |
